...
export class TodayPreviewEffects {
  findLocationDetails = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((response: TodayPreviewActions.FindLocationDetails) => {
        return this.http
          .get<LocationResponseData>(
            environment.locationSearchApi +
              response.payload.latt +
              ',' +
              response.payload.long
          )
          .pipe(
            map((responseData) => {
            ...
            catchError((error) => {
              return of(
                new TodayPreviewActions.ErrorLocation(error.statusText)
              );
            })
          );
      })
    ...
    this.actions$.pipe(
      ...
      switchMap((response: TodayPreviewActions.UpdateLocation) => {
        ...
        return this.http
          ...
          .pipe(
            map((responseData) => {
              return new TodayPreviewActions.UpdateWeather(
                responseData.consolidated_weather
              );
            })
          );
      })
